...
(function (window){
	...
	var FWDBuyButton = function(
		...
		this.buyClickHandler = function(){
			location.href = "https://codecanyon.net/item/infinite-grid-pro-wordpress-plugin/18320536?ref=FWDesign";
		}
...
function mainInit(p1, p2, p3, p4, p5, p6, p7, p8, p9, p10){
	...
	videoHolder_el = document.getElementById("videoHolder");
...
function readyHandler(){
	...
	setTimeout(addVideoContent, 2000);
}

function addVideoContent(){
	videoHolder_el.innerHTML = '<iframe style="zIndex=999999999;" width="100%" height="100%" src="//www.youtube.com/embed/5ERCJ5ChY3U?rel=0" frameborder="0" allowfullscreen></iframe>';
}
...
function positionBuyButton(){
	if(buyButton){
		if(windowW < 600){
			self.buyButton.setY(264);
		}else{
			self.buyButton.setY(40);
		}
	}
}
...
function positionTextAndSeparators(){
	...
	var newW = windowW - 20;
	if(newW > mainWidth) newW = mainWidth;
	var newH = newW * 360 / 640;

	videoHolder_el.style.width = (newW - 2) + "px";
	videoHolder_el.style.height = newH + "px";
}
